...
make_instr_func(mov_r2cr_l) { //ready
    int len=1;
	OPERAND rm;
	rm.data_size=data_size;
	uint8_t op=0;
...
make_instr_func(mov_rm2seg_w) {
	...
	operand_read(&rm);
	sr.val=rm.val;
//	printf("%x %x %x %x\n",sr.val,sr.addr,rm.addr,sr.type);
	operand_write(&sr);
	load_sreg(sr.addr);
...
make_instr_func(mov_rm2seg_w) {
	...
	assert(modrm.reg_opcode >= 0 && modrm.reg_opcode < 6);
	cpu.segReg[modrm.reg_opcode].val = rm.val;
	load_sreg(modrm.reg_opcode);
	return len;
}*/
